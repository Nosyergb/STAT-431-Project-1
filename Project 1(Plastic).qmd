...
```{r}
# Example: plastic count per countries by language spoken
Updated_Plastics |>
  pivot_longer(
...
Updated_Plastics |>
  filter(!is.na(lang1)) |>
  add_count(lang1, name = "lang_count") |>
  filter(lang_count >= 20) |>
  ggplot(aes(x = lang1, y = total_plastic)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
Based on this secondary graph, we can see that countries with English reported as their primary language easily produced the most plastic waste from 2019-2020. French-speaking countries follow in plastic waste, though there is a noticeable drop-off in the maximum values after these groups.