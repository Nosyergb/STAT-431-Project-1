...
changes <- map_dbl(plastic_types,
                   function(type) {
                      plastics |>
                        group_by(year) |>
                        summarize(total = sum(type, na.rm = TRUE),
                                  .groups = "drop") |>
                        arrange(year) |>
                        pull(total) |>
                      diff()
...
?pLanguages <- map(codes, collect_api_data)
...
# Aggregate plastic data to country-year level
...
# Reshape + summarize for plotting
plastics_country |>
  pivot_longer(
...
```{r}
# Research question: How does the distribution of plastic waste types change with the different income levels of countries?
...
filter(str_detect(title, 'Toy Story' | str_detect(title, 'Cars')))
# Chi-Squared Test of Plastic Type by Wealth Group

A Pearson's chi-squared test of independence showed that the relationship between plastic type and wealth group is statistically significant (X\^2(24) = 142405, p-value \< 0.05), indicating that the distribution of plastics types does differ between wealth groups.
